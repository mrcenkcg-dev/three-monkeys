...
def auto_poster_agent():
    """
    Runs continuously in a background thread.
    Picks top/trending deals from hub.db, formats social teasers with our hub link,
    and pushes them to connected webhooks (Discord/Telegram/Social Automation).
    """
    print("📣 Auto-Poster Traffic Agent initialized and running...")
    HUB_URL = "https://free-monkey-system.onrender.com"
    WEBHOOK_URL = os.environ.get("DISCORD_WEBHOOK_URL", "") # Add your webhook in Render Env Vars if desired
    ...
    while True:
        try:
            ...
            # Grab top 3 deals by clicks or recency
            ...
            if deals:
                ...
                # If a webhook URL is defined in Render Environment Variables, dispatch it
                if WEBHOOK_URL:
                    ...
                    with urllib.request.urlopen(req, timeout=10) as resp:
                        print("✅ Auto-Poster: Promo payload successfully dispatched to Webhook!")
...
# Start both AI Agents in background threads
seeder_thread = threading.Thread(target=auto_seeder_agent, daemon=True)
seeder_thread.start()
