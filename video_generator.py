...
def create_tech_deal_short(product_title, discount_price, original_price):
    ...
    # Dark Blue Background (1080x1920 Vertical)
    background = ColorClip(size=(1080, 1920), color=(15, 23, 42), duration=duration)
    
    # Headline
    header = (TextClip(text="🔥 TECH DEAL ALERT 🔥", font_size=55, color="#F59E0B", font="Arial-Bold")
              .with_duration(duration)
              .with_position(("center", 350)))
    
    # Product Title
    title = (TextClip(text=product_title, font_size=45, color="white", font="Arial-Bold")
             .with_duration(duration)
             .with_position(("center", 650)))
             
    # Price & Discount
    price_text = f"NOW {discount_price}  (Was {original_price})"
    price = (TextClip(text=price_text, font_size=55, color="#10B981", font="Arial-Bold")
             .with_duration(duration)
             .with_position(("center", 950)))
             
    # Call To Action
    cta = (TextClip(text="Check Link in Description 👇", font_size=40, color="#9CA3AF", font="Arial")
           .with_duration(duration)
           .with_position(("center", 1300)))
    
    # Combine layers into video
